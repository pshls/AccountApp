VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} KARTELAPELATON 
   Caption         =   "KARTELAPELATON"
   ClientHeight    =   8340
   ClientLeft      =   45
   ClientTop       =   375
   ClientWidth     =   9045
   OleObjectBlob   =   "KARTELAPELATON.frx":0000
   StartUpPosition =   1  'CenterOwner
End
Attribute VB_Name = "KARTELAPELATON"
...
Dim test As Worksheet
    ...
    Sheets("PELATES").Copy After:=Sheets(Sheets.Count)
...
If TextBox1.Value <> vbNullString Then
    .Range("A:I").AutoFilter Field:=1, Criteria1:=TextBox1.Value, Operator:=xlFilterValues
    ...
    .Range("A:I").AutoFilter Field:=2, Criteria1:=TextBox5.Value, Operator:=xlFilterValues
...
End If
   ...
   If TextBox3.Value & TextBox4.Value <> vbNullString Then
    .Range("A:I").AutoFilter Field:=3, Criteria1:=">=" & FMDATE, Criteria2:="<=" & TODATE, Operator:=xlFilterValues
...
End If
 ...
 If CheckBox1.Value = True Then
  .Range("A:I").AutoFilter Field:=7, Criteria1:=">0", Operator:=xlFilterValues
   End If
   
   
    If CheckBox2.Value = True Then
  .Range("A:I").AutoFilter Field:=8, Criteria1:=">0", Operator:=xlFilterValues
   End If
...
Dim testB As Worksheet



    Sheets("PARAGELIES").Copy After:=Sheets(Sheets.Count)
    Set testB = ActiveSheet
    testB.Name = "TEST3"



With testB
    LastCell = .Range("A" & testB.Rows.Count).End(xlUp).Row
    Set RngOne = .Range("A2:A" & LastCell)
End With
...
Dim oRowB As Range, rngB As Range
    Dim myRowsB As Range
    
    
    
  
    With testB
        Set myRowsB = Intersect(.Range("A:A").EntireRow, .UsedRange)
        If myRowsB Is Nothing Then Exit Sub
    End With

    For Each oRowB In myRowsB.Columns(1).Cells
        If oRowB.EntireRow.Hidden Then
            If rngB Is Nothing Then
                Set rngB = oRowB
            Else
                Set rngB = Union(rngB, oRowB)
            End If
        End If
    Next

    If Not rngB Is Nothing Then rngB.EntireRow.Delete
...
testB.AutoFilterMode = False
   ...
   With Sheets("TEST1")
    ...
    .Range("A1:I" & lastrow + 1).Copy wks.Range("A" & wks.Rows.Count).End(xlUp)
   ...
   With Sheets("TEST3")
    ...
    .Range("F" & lastrow2 + 1).Value = WorksheetFunction.Sum(.Range("F1:F" & lastrow2))

    .Range("A1:F" & lastrow2 + 1).Copy wks.Range("A" & wks.Rows.Count).End(xlUp).Offset(3)
